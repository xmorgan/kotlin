...
//        assert(builtIns.builtIns.builtInsModule === delegate.moduleDescriptor)
    ...
    private fun checkIsFunctionInterface(idSig: IdSignature): Boolean {
        ...
        if (publicSig.packageFqName !in functionalPackages) return false
        if (publicSig.declarationFqName.isEmpty()) return false

        return functionPattern.matcher(publicSig.firstNameSegment).find()
    }
