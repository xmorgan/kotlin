...
abstract class Fir2IrBindableSymbol<out D : DeclarationDescriptor, B : IrSymbolOwner>(
    ...
    private var _descriptor: D? = null

    override fun bind(owner: B) {
        if (_owner == null) {
            _owner = owner
            _descriptor = when (owner) {
                is IrEnumEntry -> WrappedEnumEntryDescriptor()
                is IrClass -> WrappedClassDescriptor()
                is IrConstructor -> WrappedClassConstructorDescriptor()
                is IrFunction -> when {
                ...
                is IrVariable -> WrappedVariableDescriptor()
                is IrValueParameter -> WrappedValueParameterDescriptor()
                is IrTypeParameter -> WrappedTypeParameterDescriptor()
                is IrProperty -> if (containerSource != null) {
                    WrappedPropertyDescriptorWithContainerSource(containerSource)
                } else {
                    WrappedPropertyDescriptor()
                }
                is IrField -> WrappedFieldDescriptor()
    ...
    override val descriptor: D
        get() = _descriptor ?: throw IllegalStateException("Symbol is unbound")
}
